...
<div class="wrapper">
    ...
    <div class="main-panel">
        ...
        <div class="content">
                    ...
                    <div class="col-md-12">
                    	<div class="row">
                    		...
                    		<div class="btn-group" style="float: right; padding-bottom: 15px;padding-right: 5px;">
                                <a href="addClass" type="button" class="btn btn-default">
                                  <i class="ti-plus"></i>&nbsp;&nbsp;Mở lớp
                                </a>
                            </div>
                    	</div>
                    	<div class="card">
                            <div class="header" style="float: left;">
                                <h4 class="title">Danh sách lớp học</h4>
                                <p class="category"></p>
                            </div>
                            <div class="btn-group" style="float: right;width: 320px; padding-top: 13px;padding-right: 17px;padding-bottom: 13px;">
                                <form action="loadBasedonCourse" method="post">
                                	<input type="hidden" id="course" value="${course}">
                                    <select style="float: left; width: 72%" id="courseID" name="courseID" class="form-control border-input">
                                        <option class="dropdown" value="0">Tất cả</option>
                                        <c:forEach items="${listCourse}" var="listCourse">
                                        	<option class="dropdown" value="${listCourse.course_id}">${listCourse.coursename}</option>
                                        </c:forEach>
                                    </select>
                            ...
                            <form action="deleteClass" id="form1" method="post" onsubmit="return checkCheckBoxes(this)">
	                                    	...
	                                    	<c:forEach items="${listClass}" var="listClass">
	                                    		<tr>
	                                    			<td>
	                                    				...
	                                    				<input type="checkbox" name="classDel[]" value="${listClass.class_id}"/>
	                                    				</div>
	                                    			</td>
	                                    			<td>${listClass.classname}</td>
		                                            <td>${listClass.startday}</td>
		                                            <td>${listClass.endday}</td>
		                                            <td>${listClass.timestudy}</td>
		                                            <td>${listClass.location}</td>
		                                            <td>${listClass.testday}</td>
		                                            <td><a href="viewClassInfo?classID=${listClass.class_id}" 
		                                            style="font-size: 16px; color: black; padding-left: 30px;" class="ti-pencil-alt"></a></td>
		                                            <td><a href="viewListStudent?classID=${listClass.class_id}" 
		                                            style="font-size: 16px; color: #d45f31; padding-left: 30px;" class="ti-align-justify">&nbsp;${listClass.number_of_students}&nbsp;HV</a></td>
	                                        	</tr>
	                                    	</c:forEach>
	                                    </tbody>
	                                </table> 
        	...
        	<a href="viewRegisteredStudents" style="color: black; padding-left: 20px; font-size: 16px;">Danh sách học viên đăng kí</a>
        </div>
...
(function () {
    ...
    k = document.getElementById('course').value; //lấy giá trị trong hidden form
    var x = document.getElementById("courseID").options.length;
          for(var i=0; i < x; i++)
          {
            if(document.getElementById('courseID').options[i].value == k) {
              document.getElementById('courseID').selectedIndex = i;
              return;
            }
          }  
	...
	function checkCheckBoxes(theForm) {
		var test = document.getElementsByName("classDel[]");
